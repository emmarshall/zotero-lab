...
function overview_card()
  ...
  local ocol = pandoc.Div({}, { class = "col-sm-8" })
  ...
  table.insert(objectives_div.content,
    pandoc.RawBlock("html", "<h3 class='card-title'>Learning Objectives</h3>"))
  ...
  table.insert(overview.content,
    pandoc.RawBlock("html", "<h1 class='card-header'>Summary</h1>"))
...
local accordion_titles = {
  ...
  ["hint"] = "Give me a hint",
  ["challenge"] = "Show me the challenge",
...
challenge_block = function(el)
  -- The challenge blocks no longer contain challenges nested inside. Instead,
  -- the challenges (and hints) are piled at the end of the block, so series of
  -- challenge/challenges need to be separated.

  -- The challenge train is a list to contain all the divs
  local challenge_train = pandoc.List:new()
  -- If the challenge contains multiple challenges or hints, we need to indicate
  -- that the following challenges/challenges are continuations.
...
handle_our_divs = function(el)
  ...
  -- Accordion blocks:
  --
  -- Instructor Notes, challenges, Hints, and Spoilers are all blocks
  -- that are contained in accordion blocks.
  -- For historical reasons, challenges are normally embedded
  ...
  -- Challenge blocks:
  --
  -- Challenge blocks no longer contain challenges, so the challenges (and hints)
...
--    way the activity appear in the HTML site
